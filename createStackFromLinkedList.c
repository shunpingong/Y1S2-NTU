...
{
    int tempitem=0;
    Stack temp; //temp stack to store odd values
    temp.ll.head =NULL;
    temp.ll.size=0;
    ...
    //transfer from temp stack back to initial stack
...
void removeEvenValues(Stack *s) //Pointer implementation (need malloc memory space first for pointer)
{
	int size = s->ll.size;
	int temp =0;
