...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	int splitsize, count=0;

	ListNode *cur = ll->head;

	splitsize = ll->size - (ll->size /2);

	if (splitsize <1) return;

	else{
		while (count!= splitsize-1){
			cur = cur->next;
			count++;
		}
	...
	resultFrontList->size = splitsize;
	resultBackList->size = ll->size - splitsize;
	cur->next = NULL;
	ll->size = 0;
	ll->head = NULL;
	}
...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	...
	int splitSize,count;
	count =0;
	splitSize = ll->size - (ll->size /2); //ll->size / 2 = 7/2 = 3 --> 7 - 3 =4
	cur = ll->head;
